...
The current standalone skill is published in
[`liujianpeng678-hash/architecture-review-plus`](https://github.com/liujianpeng678-hash/architecture-review-plus/tree/codex/sync-latest-audit-skill/skills/architecture-review).
Clone the repository and copy its **`skills/architecture-review/`** directory into your
agent's skills directory (for Codex: `~/.codex/skills/architecture-review/`). If an older
copy is installed, replace that copy rather than keeping duplicate registrations.
Restart or open a new session and invoke **`$architecture-review`**.

Keep all scripts, references and assets together. This standalone skill uses its own
engine; the repository-root Plus bridge/service is a separate implementation. Project
audit data lives in the project's `.codemap/` directory, not inside the installed skill.
