...
Perform the existing full `init` workflow: decompose, scan, audit every
module with sufficient coverage and declared reviewer mode, synthesize themes, render and stamp the baseline. Structure-only mode does not
...
context, never the only source of truth. After evidence-backed re-audits with declared reviewer mode, scan, render and
